...
class CarController(CarControllerBase):
    ...
    def update(self, CC, CS, now_nanos):
        ...
        ### STEER ###
        steer_hud_alert = 1 if hud_control.visualAlert in (
            VisualAlert.steerRequired, VisualAlert.ldw) else 0
        ...
        if CC.latActive:
            ...
            if not bool(CS.out.steeringPressed):
                ...
                lkas_max_torque = max(
                    ...
                    CarControllerParams.LKAS_MAX_TORQUE - 0.6 *
                    max(0, abs(CS.out.steeringTorque) -
        ...
        if self.CP.carFingerprint in (CAR.NISSAN_ROGUE, CAR.NISSAN_XTRAIL, CAR.NISSAN_ALTIMA) and pcm_cancel_cmd:
            can_sends.append(nissancan.create_acc_cancel_cmd(
                self.packer, self.car_fingerprint, CS.cruise_throttle_msg))
        ...
        if self.CP.carFingerprint in (CAR.NISSAN_LEAF, CAR.NISSAN_LEAF_IC) and self.frame % 2 == 0:
            can_sends.append(nissancan.create_cancel_msg(
                self.packer, CS.cancel_msg, pcm_cancel_cmd))

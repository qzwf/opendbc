             ...
             ret.longitudinalTuning.kiBP = [0.]
              ret.longitudinalTuning.kiV = [0.5]
               ...
               if not ret.radarUnavailable and DBC[candidate][Bus.radar] == RADAR.DELPHI_MRR:
                ...
                if CAN.main >= 4:
                    cfgs.insert(0, get_safety_config(
                ...
                if ret.flags & FordFlags.CANFD:
                    ...
                    pscm_config = next((fw for fw in car_fw if fw.ecu ==
                                       Ecu.eps and b'\x22\xDE\x01' in fw.request), None)
                    if pscm_config:
                        if len(pscm_config.fwVersion) != 24:
                            ...
                            # Traffic Jam Assist
                            config_tja = pscm_config.fwVersion[7]
                            # Lane Centering Assist
                            config_lca = pscm_config.fwVersion[8]
                            if config_tja != 0xFF or config_lca != 0xFF:
                                carlog.error(
                                    'dashcamOnly: Car lacks required lateral control APIs')
                                ret.dashcamOnly = True

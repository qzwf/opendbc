...
# slightly above steering pressed allows some resistance when changing lanes
MAX_LTA_DRIVER_TORQUE_ALLOWANCE = 150
...
class CarController(CarControllerBase):
    ...
    def update(self, CC, CS, now_nanos):
        ...
        lat_active = CC.latActive and abs(
            CS.out.steeringTorque) < MAX_USER_TORQUE
        ...
        if self.CP.flags & ToyotaFlags.SECOC.value:
            if CS.secoc_synchronization['RESET_CNT'] != self.secoc_prev_reset_counter:
                ...
                expected_mac = build_sync_mac(self.secoc_key, int(
                    CS.secoc_synchronization['TRIP_CNT']), int(CS.secoc_synchronization['RESET_CNT']))
        ...
        if self.CP.steerControlType == SteerControlType.angle:
            ...
            if self.frame % 2 == 0:
                ...
                apply_angle = apply_std_steer_angle_limits(
                    apply_angle, self.last_angle, CS.out.vEgoRaw, self.params)

                if not lat_active:
                    apply_angle = CS.out.steeringAngleDeg + CS.out.steeringAngleOffsetDeg

                self.last_angle = clip(
                    apply_angle, -MAX_LTA_ANGLE, MAX_LTA_ANGLE)
        ...
        if self.frame % 2 == 0 and self.CP.carFingerprint in TSS2_CAR:
            ...
            if self.CP.flags & ToyotaFlags.SECOC.value:
                lta_steer_2 = toyotacan.create_lta_steer_command_2(
        ...
        if self.CP.openpilotLongitudinalControl:
            if self.frame % 3 == 0:
                ...
                if CC.longActive:
                    pcm_accel_cmd = rate_limit(
                        pcm_accel_cmd, self.prev_accel, ACCEL_WINDDOWN_LIMIT, ACCEL_WINDUP_LIMIT)
                ...
                accel_due_to_pitch = math.sin(
                    min(self.pitch.x, 0.0)) * ACCELERATION_DUE_TO_GRAVITY
                ...
                if not self.CP.flags & ToyotaFlags.SECOC.value:
                    a_ego_blended = float(
                        np.interp(CS.out.vEgo, [1.0, 2.0], [CS.gvc, CS.out.aEgo]))
                else:
                ...
                if CC.longActive:
                    ...
                    self.long_pid.i -= ACCEL_PID_UNWIND * \
                        float(np.sign(self.long_pid.i))
                ...
                net_acceleration_request_min = min(
                    actuators.accel + accel_due_to_pitch, net_acceleration_request)
                ...
                pcm_accel_cmd = clip(
                    pcm_accel_cmd, self.params.ACCEL_MIN, self.params.ACCEL_MAX)
            ...
            if pcm_cancel_cmd:
                if self.CP.carFingerprint in UNSUPPORTED_DSU_CAR:
                    can_sends.append(
                        toyotacan.create_acc_cancel_command(self.packer))
                else:
                    can_sends.append(toyotacan.create_accel_command(
                        self.packer, 0, pcm_cancel_cmd, True, False, lead, CS.acc_type, False, self.distance_button))

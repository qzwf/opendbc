      ...
      return CarControllerParams.NIDEC_ACCEL_MIN, np.interp(current_speed, ACCEL_MAX_BP, ACCEL_MAX_VALS)
              ...
              ret.lateralTuning.pid.kf = 0.00006  # conservative feed-forward
               ...
               if candidate in HONDA_BOSCH:
                    ...
                    if candidate in HONDA_BOSCH_RADARLESS:
                        # stock uses -4.0 m/s^2 once stopped but limited by safety model
                        ret.stopAccel = CarControllerParams.BOSCH_ACCEL_MIN
                else:
                ...
                if candidate == CAR.HONDA_CIVIC:
                    ...
                    if candidate == CAR.HONDA_HRV:
                        ...
                        ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [
                            [0.8], [0.24]]  # TODO: can probably use some tuning
                    ...
                    if candidate == CAR.HONDA_ODYSSEY_CHN:
                        ret.lateralParams.torqueBP, ret.lateralParams.torqueV = [[0, 32767], [
                            0, 32767]]  # TODO: determine if there is a dead zone at the top end
                    else:
                        # TODO: determine if there is a dead zone at the top end
                        ret.lateralParams.torqueBP, ret.lateralParams.torqueV = [
                            [0, 4096], [0, 4096]]
                    ...
                    ret.lateralParams.torqueBP, ret.lateralParams.torqueV = [
                        [0, 4096], [0, 4096]]
                    ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [
                        [0.6], [0.18]]  # TODO: can probably use some tuning
        ...
        def init(CP, can_recv, can_send):
            if CP.carFingerprint in (HONDA_BOSCH - HONDA_BOSCH_RADARLESS) and CP.openpilotLongitudinalControl:
                disable_ecu(can_recv, can_send, bus=CanBus(CP).pt,
                            addr=0x18DAB0F1, com_cont_req=b'\x28\x83\x03')
